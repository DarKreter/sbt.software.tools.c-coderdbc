...
const char *extend_func_body =
    "// To compile this function you need to typedef 'bitext_t' and "
    "'ubitext_t'\n"
...
void CiMainGenerator::Generate(DbcMessageList_t &dlist,
                               const FsDescriptor_t &fsd) {
  p_dlist = &dlist;
...
void CiMainGenerator::Gen_MainHeader() {
  ...
  if (fdesc->start_info.size() > 0) {
    ...
    fwriter->AppendLine("// " + std::regex_replace(fdesc->start_info,
                                                   std::regex("\n"), "\n// "));
  }
  ...
  // Add description and warning in comment
  fwriter->AppendLine(StrPrint("/**\n"
                               "* !!!WARNING!!!\n"
                               "* This file is autogenerated, any changes will "
                               "not be retained after next\n"
                               "* generation.\n"
                               "*\n"
                               "* @brief In this file are struct definitions, "
                               "Pack and Unpack function\n"
                               "* declarations.\n"
                               "*/"),
                      2);
  ...
  fwriter->AppendLine(StrPrint("struct CAN_STRUCT_SAMPLE_t {};"), 2);

  fwriter->AppendLine("// DBC file version");
  fwriter->AppendLine(StrPrint("#define %s (%uU)", fdesc->verhigh_def.c_str(),
                               p_dlist->ver.hi));
  fwriter->AppendLine(
      StrPrint("#define %s (%uU)", fdesc->verlow_def.c_str(), p_dlist->ver.low),
      2);

  //  fwriter->AppendLine("// include current dbc-driver compilation config");
  //  fwriter->AppendLine(StrPrint("#include \"%s-config.h\"",
  //  fdesc->drvname.c_str()), 2);
  ...
  for (size_t num = 0; num < sigprt->sigs_expr.size(); num++) {
    ...
    if (m.CommentText.size() > 0) {
      // replace all '\n' on "\n //" for c code comment text
      fwriter->AppendLine(
          "// " + std::regex_replace(m.CommentText, std::regex("\n"), "\n// "));
    }

    fwriter->AppendLine(StrPrint("// def @%s CAN Message (%-4d %#x)",
                                 m.Name.c_str(), m.MsgID, m.MsgID));
    fwriter->AppendLine(
        StrPrint("#define %s_IDE (%uU)", m.Name.c_str(), m.IsExt));
    fwriter->AppendLine(
        StrPrint("#define %s_DLC (%uU)", m.Name.c_str(), m.DLC));
    fwriter->AppendLine(
        StrPrint("#define %s_CANID (%#x)", m.Name.c_str(), m.MsgID));

    if (m.Cycle > 0) {
      fwriter->AppendLine(
          StrPrint("#define %s_CYC (%dU)", m.Name.c_str(), m.Cycle));
    }
    ...
    fwriter->AppendLine(
        StrPrint("struct %s_t: CAN_STRUCT_SAMPLE_t", m.Name.c_str()));
    ...
    if (m.RollSig != nullptr) {
      ...
      fwriter->AppendLine(StrPrint("#endif // %s", fdesc->useroll_def.c_str()),
                          2);
    }
    ...
    fwriter->AppendLine(StrPrint("#endif // %s", fdesc->usebits_def.c_str()),
                        2);
  ...
  for (size_t num = 0; num < sigprt->sigs_expr.size(); num++) {
    ...
    fwriter->AppendLine(
        StrPrint("[[nodiscard]] %s_t Unpack_%s(const uint8_t* _d);",
                 m.Name.c_str(), m.Name.c_str()));

    fwriter->AppendLine(StrPrint("#ifdef %s", fdesc->usesruct_def.c_str()));

    fwriter->AppendLine(
        StrPrint("void Pack_%s(%s_t* _m, __CoderDbcCanFrame_t__* cframe);",
                 m.Name.c_str(), m.Name.c_str()));
    ...
    fwriter->AppendLine(StrPrint("#endif // %s", fdesc->usesruct_def.c_str()),
                        2);
  }
...
void CiMainGenerator::Gen_MainSource() {
  ...
  fwriter->AppendLine(StrPrint("#include \"%s\"", fdesc->core_h.fname.c_str()),
                      3);
  ...
  fwriter->AppendLine(
      StrPrint(
          "/**\n"
          " * !!!WARNING!!!\n"
          " * This file is autogenerated, any changes will not be retained "
          "after next\n"
          " * generation.\n"
          " *\n"
  ...
  fwriter->AppendLine(
      StrPrint("#include \"%s-fmon.h\"", fdesc->drvname.c_str()), 2);
  ...
  for (size_t num = 0; num < sigprt->sigs_expr.size(); num++) {
    ...
    // first function
    fwriter->AppendLine(
        StrPrint("%s_t Unpack_%s(const uint8_t* _d)\n{ \n%s_t _m;",
                 m.Name.c_str(), m.Name.c_str(), m.Name.c_str()));
    ...
    // second function
    fwriter->AppendLine(
        StrPrint("uint32_t Pack_%s(%s_t* _m, __CoderDbcCanFrame_t__* cframe)",
                 m.Name.c_str(), m.Name.c_str()));
...
void CiMainGenerator::Gen_ConfigHeader() {
  ...
  fwriter->AppendLine("/* "
                      "--------------------------------------------------------"
                      "----------------- *");
  fwriter->AppendLine("  This define enables using CAN message structs with "
                      "bit-fielded signals");
  fwriter->AppendLine("  layout.");
  fwriter->AppendLine("");
  fwriter->AppendLine("  Note(!): bit-feild was not tested properly. */");
  fwriter->AppendLine("");
  fwriter->AppendLine(StrPrint("/* #define %s */", fdesc->usebits_def.c_str()),
                      3);

  fwriter->AppendLine("/* "
                      "--------------------------------------------------------"
                      "----------------- *");
  fwriter->AppendLine("  This macro enables using CAN message descriptive "
                      "struct packing functions");
  fwriter->AppendLine("  (by default signature of pack function intakes a few "
                      "simple typed params");
  fwriter->AppendLine("  for loading data, len, etc). To compile you need to "
                      "define the struct");
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine("  This struct definition have to be placed (or be "
                      "included) in dbccodeconf.h */");
  fwriter->AppendLine("");
  fwriter->AppendLine(StrPrint("/* #define %s */", fdesc->usesruct_def.c_str()),
                      3);
  ...
  fwriter->AppendLine("  operations will be made on this signal value as well "
                      "as unpack from payload.");
  fwriter->AppendLine("");
  ...
  fwriter->AppendLine("  If only offset != 0 is true then the type of '_phys' "
                      "signal is the same");
  fwriter->AppendLine(
      "  as '_ro' signal. In other case the type will be @sigfloat_t which");
  fwriter->AppendLine("  have to be defined in user dbccodeconf.h");
  fwriter->AppendLine("");
  fwriter->AppendLine("  2. In pack function '_ro' signal will be rewritten by "
                      "'_phys' signal, which");
  fwriter->AppendLine(
      "  requires from user to use ONLY '_phys' signal for packing frame");
  fwriter->AppendLine("");
  fwriter->AppendLine("  3. In unpack function '_phys' signal will be written "
                      "by '_ro' signal.");
  fwriter->AppendLine(
      "  User have to use '_phys' signal to read physical value. */");
  fwriter->AppendLine("");
  fwriter->AppendLine(
      StrPrint("/* #define %s */", fdesc->usesigfloat_def.c_str()), 3);

  fwriter->AppendLine("/* "
                      "--------------------------------------------------------"
                      "----------------- *");
  fwriter->AppendLine("  Note(!) that the \"canmonitorutil.h\" must be "
                      "accessed in include path:");
  fwriter->AppendLine("");
  fwriter->AppendLine("  This macro adds:");
  fwriter->AppendLine("");
  fwriter->AppendLine("  - monitor field @mon1 to message struct");
  fwriter->AppendLine("");
  fwriter->AppendLine("  - capture system tick in unpack function and save "
                      "value to mon1 field");
  fwriter->AppendLine("  to provide to user better missing frame detection "
                      "code. For this case");
  fwriter->AppendLine("  user must provide function declared in "
                      "canmonitorutil.h - GetSysTick()");
  fwriter->AppendLine("  which may return 1ms uptime.");
  fwriter->AppendLine("");
  fwriter->AppendLine("  - calling function FMon_***  (from 'fmon' driver) "
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine(StrPrint("/* #define %s */", fdesc->usemon_def.c_str()),
                      3);
  ...
  fwriter->AppendLine("/* "
                      ...
                      "----------------- *");
  fwriter->AppendLine(
      StrPrint("  When monitor using is enabled (%s) and define below",
               fdesc->usemon_def.c_str()));
  fwriter->AppendLine("  uncommented, additional signal will be added to "
                      "message struct. ***_expt:");
  fwriter->AppendLine("  expected rolling counter, to perform monitoring "
                      "rolling counter sequence");
  fwriter->AppendLine("  automatically (result may be tested in dedicated "
                      "Fmon_*** function) */");
  fwriter->AppendLine("");
  fwriter->AppendLine(StrPrint("/* #define %s */", fdesc->useroll_def.c_str()),
                      3);
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine("  The signal which may be marked as checksum signal "
                      "must have substring");
  ...
  fwriter->AppendLine("  (canmonitorutil.h) function as is, the best use case "
                      "is to define 'enum");
  fwriter->AppendLine(
      "  DbcCanCrcMethods' in canmonitorutil.h file with all possible");
  fwriter->AppendLine("  checksum algorithms (e.g. XOR8, XOR4 etc)");
  fwriter->AppendLine("");
  fwriter->AppendLine("  - \"3\": optional value that will be passed to "
                      "GetFrameHash as integer value");
  fwriter->AppendLine("");
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine(StrPrint("/* #define %s */", fdesc->usecsm_def.c_str()),
                      2);
...
void CiMainGenerator::Gen_FMonHeader() {
  ...
  fwriter->AppendLine("// DBC file version");
  fwriter->AppendLine(StrPrint("#define %s_FMON (%uU)",
                               fdesc->verhigh_def.c_str(), p_dlist->ver.hi));
  fwriter->AppendLine(StrPrint("#define %s_FMON (%uU)",
                               fdesc->verlow_def.c_str(), p_dlist->ver.low),
                      2);

  fwriter->AppendLine(
      StrPrint("#include \"%s-config.h\"", fdesc->drvname.c_str()), 2);
...
void CiMainGenerator::Gen_FMonSource() {
  ...
  fwriter->AppendLine(StrPrint("#include \"%s\"", fdesc->fmon_h.fname.c_str()),
                      2);
  // put diagmonitor ifdef selection for including @drv-fmon header
  // with FMon_* signatures to call from unpack function
  fwriter->AppendLine(StrPrint("#ifdef %s", fdesc->usemon_def.c_str()), 2);
...
void CiMainGenerator::Gen_CanMonUtil() {
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine("  // @timeout_cycle keeps maximum timeout for frame, "
                      "user responsibility");
  fwriter->AppendLine("  // to init this field and use it in missing frame "
                      "monitoring function");
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine("  // setting up @checksum_error bit indicates checksum "
                      "checking failure.");
  fwriter->AppendLine("  // Bit is not clearing automatically!");
  ...
  fwriter->AppendLine(
      "uint8_t GetFrameHash(const uint8_t* data_ptr, uint8_t len, uint32_t "
      "msgid, DbcCanCrcMethods type, uint32_t option);");
  fwriter->AppendLine("");
  fwriter->AppendLine("/* "
                      "--------------------------------------------------------"
                      "--------------------- */");
  fwriter->AppendLine("// this function will be called when unpacking is "
                      "performing. Value will be saved");
...
void CiMainGenerator::Gen_DbcCodeConf() {
  ...
  fwriter->AppendLine("");
  fwriter->AppendLine("// if you need to allocate rx and tx messages structs "
                      "put the allocation macro here");
...
void CiMainGenerator::WriteSigStructField(const SignalDescriptor_t &sig,
                                          bool bits, size_t padwidth) {
  if (sig.CommentText.size() > 0) {
    fwriter->AppendLine("  // " + std::regex_replace(sig.CommentText,
                                                     std::regex("\n"),
                                                     "\n  // "));
  }

  if (sig.ValueText.size() > 0) {
    fwriter->AppendLine("  // " + std::regex_replace(sig.ValueText,
                                                     std::regex("\n"),
                                                     "\n  // "));
  }
  ...
  if (!sig.IsSimpleSig) {
    ...
    // own 'shadow' (_phys) copies, the problem with intermediate type (not
    // simpe and not double) is that the x = ***_toS(x) takes place in each
    // Pack_* call the signals which are not changing from Pack_* to Pack_* will
    // change its values (!)
    ...
    if (sig.IsDoubleSig) {
      ...
      fwriter->AppendLine(StrPrint("  %s %s;",
                                   PrintType((int)sig.TypePhys).c_str(),
                                   sig.NameFloat.c_str()));
    }

    fwriter->AppendLine(
        StrPrint("#endif // %s", fdesc->usesigfloat_def.c_str()), 2);
  }
}
...
void CiMainGenerator::WriteUnpackBody(const CiExpr_t *sgs) {
  for (size_t num = 0; num < sgs->to_signals.size(); num++) {
    ...
    if (sgs->msg.Signals[num].Signed) {
      fwriter->AppendLine(StrPrint("  _m.%s = %s(( %s ), %d);", sname,
                                   ext_sig_func_name, expr.c_str(),
                                   (int32_t)sgs->msg.Signals[num].LengthBit));
    } else {
    ...
    if (!sgs->msg.Signals[num].IsSimpleSig) {
      fwriter->AppendLine(
          StrPrint("#ifdef %s", fdesc->usesigfloat_def.c_str()));

      if (sgs->msg.Signals[num].IsDoubleSig) {
        // for double signals (sigfloat_t) type cast
        fwriter->AppendLine(
            StrPrint("  _m.%s = (sigfloat_t)(%s_%s_fromS(_m->%s));",
                     sgs->msg.Signals[num].NameFloat.c_str(),
                     fdesc->DRVNAME.c_str(), sname, sname));
      } else {
  ...
  fwriter->AppendLine(StrPrint("#ifdef %s", fdesc->usemon_def.c_str()));
  fwriter->AppendLine(StrPrint("  _m.mon1.dlc_error = (dlc_ < %s_DLC);",
                               sgs->msg.Name.c_str()));
  ...
  if (sgs->msg.RollSig != nullptr) {
    ...
    fwriter->AppendLine(StrPrint(
        "  _m.mon1.roll_error = (_m.%s != _m.%s_expt);",
        sgs->msg.RollSig->Name.c_str(), sgs->msg.RollSig->Name.c_str()));
    fwriter->AppendLine(StrPrint("  _m.%s_expt = (_m.%s + 1) & (0x%02XU);",
                                 sgs->msg.RollSig->Name.c_str(),
                                 sgs->msg.RollSig->Name.c_str(),
                                 (1 << sgs->msg.RollSig->LengthBit) - 1));
    // Put rolling monitor here
    fwriter->AppendLine(StrPrint("#endif // %s", fdesc->useroll_def.c_str()),
                        2);
  }
  ...
  if (sgs->msg.CsmSig != nullptr) {
    ...
    fwriter->AppendLine(
        StrPrint("  _m.mon1.csm_error = (((uint8_t)GetFrameHash(_d, %s_DLC, "
                 "%s_CANID, %s, %d)) != (_m.%s));",
                 sgs->msg.Name.c_str(), sgs->msg.Name.c_str(),
                 sgs->msg.CsmMethod.c_str(), sgs->msg.CsmOp,
                 sgs->msg.CsmSig->Name.c_str()));
    fwriter->AppendLine(StrPrint("#endif // %s", fdesc->usecsm_def.c_str()), 2);
  }

  auto Fmon_func = "FMon_" + sgs->msg.Name + "_" + fdesc->drvname;

  fwriter->AppendLine(StrPrint("  %s(&_m.mon1, %s_CANID);", Fmon_func.c_str(),
                               sgs->msg.Name.c_str()));
...
void CiMainGenerator::WritePackStructBody(const CiExpr_t *sgs) {
  ...
  fwriter->AppendLine(
      StrPrint("  cframe->MsgId = %s_CANID;", sgs->msg.Name.c_str()));
  fwriter->AppendLine(
      StrPrint("  cframe->DLC = %s_DLC;", sgs->msg.Name.c_str()));
  fwriter->AppendLine(
      StrPrint("  cframe->IDE = %s_IDE;", sgs->msg.Name.c_str(), 2));
...
void CiMainGenerator::WritePackArrayBody(const CiExpr_t *sgs) {
  ...
  //  fwriter->AppendLine(StrPrint("  *_ide = %s_IDE;", sgs->msg.Name.c_str(),
  //  2)); fwriter->AppendLine(StrPrint("  return %s_CANID;",
  //  sgs->msg.Name.c_str()));
  fwriter->AppendLine("}", 2);
}

void CiMainGenerator::PrintPackCommonText(const std::string &arrtxt,
                                          const CiExpr_t *sgs) {
  ...
  fwriter->AppendLine(
      StrPrint(
          "  uint8_t i; for (i = 0; (i < %s_DLC) && (i < 8); %s[i++] = 0);",
          sgs->msg.Name.c_str(), arrtxt.c_str()),
      2);

  if (sgs->msg.RollSig != nullptr) {
    fwriter->AppendLine(StrPrint("#ifdef %s", fdesc->useroll_def.c_str()));
    fwriter->AppendLine(StrPrint("  _m->%s = (_m->%s + 1) & (0x%02XU);",
                                 sgs->msg.RollSig->Name.c_str(),
                                 sgs->msg.RollSig->Name.c_str(),
                                 (1 << sgs->msg.RollSig->LengthBit) - 1));
    fwriter->AppendLine(StrPrint("#endif // %s", fdesc->useroll_def.c_str()),
  ...
  if (sgs->msg.CsmSig != nullptr) {
    ...
    fwriter->AppendLine(StrPrint("#ifdef %s", fdesc->usecsm_def.c_str()));
    fwriter->AppendLine(
        StrPrint("  _m->%s = 0U;", sgs->msg.CsmSig->Name.c_str()));
    fwriter->AppendLine(StrPrint("#endif // %s", fdesc->usecsm_def.c_str()), 2);
  }
  ...
  if (sgs->msg.hasPhys) {
    ...
    for (size_t n = 0; n < sgs->to_signals.size(); n++) {
      if (sgs->msg.Signals[n].IsSimpleSig == false) {
        // print toS from *_phys to original named sigint (integer duplicate of
        // signal)
        fwriter->AppendLine(StrPrint(
            "  _m->%s = %s_%s_toS(_m->%s);", sgs->msg.Signals[n].Name.c_str(),
            fdesc->DRVNAME.c_str(), sgs->msg.Signals[n].Name.c_str(),
            sgs->msg.Signals[n].NameFloat.c_str()));
      }
    }
  ...
  for (size_t i = 0; i < sgs->to_bytes.size(); i++) {
    ...
    fwriter->AppendLine(StrPrint("  %s[%d] |= %s;", arrtxt.c_str(), i,
                                 sgs->to_bytes[i].c_str()));
  }
  ...
  if (sgs->msg.CsmSig != nullptr) {
    ...
    fwriter->AppendLine(StrPrint(
        "  _m->%s = ((uint8_t)GetFrameHash(%s, %s_DLC, %s_CANID, %s, %d));",
        sgs->msg.CsmSig->Name.c_str(), arrtxt.c_str(), sgs->msg.Name.c_str(),
        sgs->msg.Name.c_str(), sgs->msg.CsmMethod.c_str(), sgs->msg.CsmOp));

    fwriter->AppendLine(StrPrint("  %s[%d] |= %s;", arrtxt.c_str(),
                                 sgs->msg.CsmByteNum,
                                 sgs->msg.CsmToByteExpr.c_str()));
